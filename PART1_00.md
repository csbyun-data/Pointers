...
double의 크기: 8 byte
```
> Windows 64bit(LLP64)는 long=4byte, Linux 64bit(LP64)는 long=8byte  
...
```c
int a = 5;     // main(){ int a = 5;} 
int *p = &a;   // void modify(int *p){} ,  main() { modify(&a);} 변경
*p = 100;      // modify(int *p) { *p = 100; }
```
```c
void modify(int *p) {
  if (p == NULL) {
...
```c
char *str = "Hello";     // 문자열 리터럴: 수정하면 UB
char arr[] = "Hello";    // 배열에 복사되므로 수정 가능
```
...
printf("%d\n", *ip); // 100
printf("%d\n", (unsigned char)*cp);  // 리틀 엔디안 기준으로 하위 1바이트가 나옴
```
...
| 포인터 크기 | 데이터 모델(LP64, LLP64, ILP32) 에 의해 결정 | `sizeof(int *)`       |
