...
이를 `간접 참조(indirection)` 또는 `역참조(dereferencing)`라고도 불리며, 포인터가 가리키는 값을 읽거나 수정하는 행위입니다
...
이 때문에 형 변환 없이 잘못된 타입으로 접근하면 이상한 결과가 나올 수 있다
...
```
📌 주소 상에서 일반적으로 스택이 가장 높은 주소를 가지며, 그 다음 힙, 데이터 순으로 배치됩니다.
...
printf("%d", a);
```
3. 두 개의 정수를 더하는 함수를 만들되, 포인터를 사용하여 결과를 호출자에게 전달하시오.
```c
// 함수 정의
