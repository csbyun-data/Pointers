...
```
> &a는 변수 a의 주소를 의미합니다  
> 포인터 p가 a를 가리킬 경우, *p는 a의 값과 같다 (즉, *p == a == 10)
...
```
###  1.0.2 메모리와 변수, 주소 개념
컴퓨터 메모리는 연속된 주소를 갖는 바이트 단위의 저장 공간입니다. 변수는 이 공간에 데이터를 저장하며,
...
### 1.0.3 변수와 포인터 관계
포인터는 변수의 주소를 저장하며, * 연산자를 사용하여 해당 주소에 저장된 값을 읽거나 변경할 수 있습니다.  
이를 간접 참조(indirection) 또는 역참조(dereferencing)라고 부릅니다.  
즉, 포인터가 가리키는 메모리 주소에 접근하여 값을 읽거나 수정하는 행위입니다.  
...
```
📌 문자열을 포인터로 선언하면 읽기 전용 메모리 영역에 쓰기 시도시 정의되지 않은 동작이 발생할 수 있습니다.
```c
char *str = "Hello";
...
printf("%d\n", *ip); // 100
printf("%d\n", *cp);  // 100, 하지만 1바이트만 읽기 때문에 저장된 정수값의 하위 1바이트만 해석
```
...
```
📌 일반적으로 스택은 가장 높은 주소 영역에, 그 아래로 힙, 데이터, 코드 순으로 배치됩니다. (하지만 이는 시스템 구조에 따라 달라질 수 있습니다.)
...
| `&`    | 주소 연산자 (`&a` → 변수 a의 주소)             | `&a`                  |
| `*`    | 간접 참조(역참조) 연산자 (`*ptr` → 주소의 값)     | `*ptr = 20;`            |
| `*ptr` | 포인터 `ptr`이 가리키는 주소에 저장된 값(역참조) | `printf("%d", *ptr);` |
| 포인터 크기 | 시스템 아키텍처(32/64bit)에 따라 다름 | `sizeof(int *)`       |
| 메모리 구조 | 코드, 데이터, 힙, 스택         | 시각적 다이어그램 참고          |
