...
| 구분     | 선언             | 설명                              | 메모리 구조         | 용도                     |  
| ------ | -------------- | ------------------------------- | -------------- | ---------------------- |  
| 배열 포인터 | `int (*p)[3];` | 정수 3개짜리 배열 하나를 가리킴 | 포인터 3개 (각각 따로) | 여러 변수의 주소 저장           |  
| 포인터 배열 | `int *p[3];`   | 정수 포인터 3개를 원소로 갖는 배열 | 배열 한 덩어리를 가리킴  | 2차원 배열, 배열 매개변수로 자주 사용 |  
...
| 2차원 배열 함수 전달 | `void f(int (*a)[COLS])` or `f(int a[][COLS])` |
