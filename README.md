...
### 11	파일 I/O
  ...
  * 11.3 구조체 배열과 포인터 활용
  * 11.4 구조체 배열을 파알애 저장
    * 11.4.1 구조체 배열의 데이터를 텍스트 파일에 저장
    * 11.4.2 fprintf()와 반복문 활용
    * 11.4.3 저장시 포멧 설계 ( CSV, 공백 구분 등) 
  * 11.5 파일에서 구조체 배열 읽어오기
    * 11.5.1 텍스트 파일에서 구조체 배열로 데이터 불러오기
    * 11.5.2 fscanf(), fgets(), sscanf() 조함
    * 파일 끝(EOF)까지 읽는 루프 구현
  * 11.6 동적 구조체 배열 + 파일 연동
  ...
  * 11.7 파일 입출력 오류 처리
    *  11.7.1 파일 열기 실패 처리
    *  11.7.2. 파일 경로, 권한 문제
    *  11.7.3 버퍼 문제 및 fflush(stdin) 사용 주의
  * 11.8 이진 파일 입출력
    * fwrite(), fread() 사용법
    * 바이너리 파일 저장과 로딩
